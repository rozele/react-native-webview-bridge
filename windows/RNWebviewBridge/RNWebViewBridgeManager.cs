...
using RNWebviewBridge.Events;
using System;
using System.Collections.Generic;
using WebViewBridgeComponent;
using Windows.UI.Xaml.Controls;
using Windows.Web.Http;
using static System.FormattableString;

namespace RNWebviewBridge
{
    /// <summary>
    /// A view manager responsible for rendering webview.
    /// </summary>
    public class ReactWebViewBridgeManager : SimpleViewManager<WebView>
    {
        ...
        private const int CommandSendToBridge = 7;
        private const int CommandGeoPermissionsGranted = 8;
        ...
        /// <summary>
        /// Instantiates the <see cref="ReactWebViewBridgeManager"/>.
        /// </summary>
        /// <param name="context">The React context.</param>
        public ReactWebViewBridgeManager(ReactContext context)
        {
            _context = context;
        }
        ...
        public override IReadOnlyDictionary<string, object> CommandsMap
        {
            get
            {
                return new Dictionary<string, object>
                {
                    ...
                    { "sendToBridge", CommandSendToBridge },
                    { "geoPermissionsGranted", CommandGeoPermissionsGranted },
                };
            }
        }
        ...
        {
            var webViewData = GetOrCreateWebViewData(view);
                        ...
                        if (method != null)
                        {
                            ...
                            else
                            {
                                throw new InvalidOperationException(
                                    Invariant($"Unsupported method '{method}' received by '{typeof(ReactWebViewBridgeManager)}'."));
                            }
                        }
                        else
                        {
        ...
        public override void ReceiveCommand(WebView view, int commandId, JArray args)
        {
            switch (commandId)
            {
                ...
                case CommandInjectJavaScript:
                    ...
                    break;                    
                case CommandSendToBridge:
                    SendToBridge(view, args[0].Value<string>());
                    break;
                case CommandGeoPermissionsGranted:
                    GrantGeolocationPermission(view);
                    break;
                default:
                    throw new InvalidOperationException(
                        Invariant($"Unsupported command '{commandId}' received by '{typeof(ReactWebViewBridgeManager)}'."));
            }
        }
        ...
        public override void OnDropViewInstance(ThemedReactContext reactContext, WebView view)
        {
            ...
            view.PermissionRequested -= OnPermissionRequested;
        }
        ...
        {
            InvokeScript(view, $"WebViewBridge.onMessage('{message}')");
        }

        private void GrantGeolocationPermission(WebView view)
        {
            var callbackId = GetWebViewData(view)?.GeolocationPermissionCallbackId;
            if (!callbackId.HasValue)
            {
                // TODO: what if the command is executed with a request?
                return;
            }

            var request = view.DeferredPermissionRequestById(callbackId.Value);
            request.Allow();
        }
        ...
        private void OnNavigationStarting(object sender, WebViewNavigationStartingEventArgs e)
        {
            ...
            var javaScriptBridge = new WebViewBridge(tag);
            javaScriptBridge.MessagePosted += OnSend;
            webView.AddWebAllowedObject("WebViewBridge", javaScriptBridge);
            webView.AddWebAllowedObject("StatusBridge", new StatusBridge(webView));
        ...
        private void OnSend(object sender, MessagePostedEventArgs e)
        {
            _context.GetJavaScriptModule<RCTEventEmitter>()
                .receiveEvent(
                    e.Tag,
                    "topChange",
                    new JObject
        ...
        private void OnPermissionRequested(WebView sender, WebViewPermissionRequestedEventArgs args)
        {
            ...
            {
                var webViewData = GetOrCreateWebViewData(sender);
                webViewData.GeolocationPermissionCallbackId = args.PermissionRequest.Id;

                var @event = new JObject
                {
                    { "type", "request_geo_permissions" },
                };

                var @params = new JObject
                {
                    { "jsonEvent", @event.ToString(Formatting.None) },
                };

                _context.GetJavaScriptModule<RCTDeviceEventEmitter>().emit("gethEvent", @params);
            }
        }

        private WebViewData GetWebViewData(WebView webView)
        {
            var result = default(WebViewData);
            if (!_webViewData.TryGetValue(webView, out result))
            {
                return null;
            }
        ...
        private WebViewData GetOrCreateWebViewData(WebView webView)
        {
            var result = default(WebViewData);
            if (!_webViewData.TryGetValue(webView, out result))
            {
                result = new WebViewData();
                _webViewData.Add(webView, result);
            }
        ...
        class WebViewData
        {
            ...
            public uint? GeolocationPermissionCallbackId { get; set; }
